...
const TOKEN_LINE_CONTINUATION = RegExp(/\\[ \t]*$/);
const TOKEN_COMMENT = RegExp(/^#.*$/);
// # dockerfile_lint  ignore | # dockerfile_lint  -  ignore  | # dockerfile_lint = ignore
const TOKEN_INLINE_IGNORE = RegExp(/^#.*dockerfile_lint[ ]*\W[ ]*ignore.*$/);
const errDockerfileNotStringArray = new Error('When using JSON array syntax, ' + 'arrays must be comprised of strings only.');
...
function parseHealthcheck(cmd) {
    ...
    if (cmdDirectiveIndex === -1 && noneDirectiveIndex === -1) {
        ...
        if (cmdDirectiveIndex > 0) {
            ...
            const cmdDirectiveOptions = words.slice(0, cmdDirectiveIndex),
                validCmdOptions = ["interval", "retries", "timeout", "start-period"];

            for (let i = 0; i < cmdDirectiveOptions.length; i++) {
                const match = /--([a-zA-Z0-9_-]+)=(\d+)/.exec(cmdDirectiveOptions[i]);
                if (!match) {
...
function parseLine(line, lineno) {
    ...
    if (line.match(TOKEN_LINE_CONTINUATION)) {
        ...
        const partialLine = line.replace(TOKEN_LINE_CONTINUATION, '', 'g');
        return {
            command: null,
